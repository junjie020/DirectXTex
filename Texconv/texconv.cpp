...
enum OPTIONS    // Note: dwOptions below assumes 64 or less options.
{
	...
	OPT_COMPRESS_MAX,
	OPT_COMPRESS_DITHER,
	OPT_RESIZE_PERCENTAGE,
	OPT_FOLLOW_SRC_FILE_TYPE,
	OPT_MAX
};
...
SValue g_pOptions[] =
{
	...
	{ L"p",             OPT_RESIZE_PERCENTAGE },
	{ L"ftsrc",         OPT_FOLLOW_SRC_FILE_TYPE},
	{ nullptr,          0 }
};
...
void PrintUsage()
{
	...
	wprintf(L"   -aw <weight>        BC7 GPU compressor weighting for alpha error metric\n"
		L"                       (defaults to 1.0)\n");
	wprintf(L"	  -p				 Set the resize percentage of the image width and height\n");
	wprintf(L"	  -ftsrc			 Make the output filetype follow the source filetype\n");
...
int __cdecl wmain(_In_ int argc, _In_z_count_(argc) wchar_t* argv[])
{
	...
	DWORD CurFileType = FileType;
	DWORD maxSize = 16384;
	float alphaWeight = 1.f;
	DWORD dwNormalMap = 0;
	float nmapAmplitude = 1.f;
	float resizePercentage = 1.f;
	...
	for (int iArg = 1; iArg < argc; iArg++)
	{
		...
		if (('-' == pArg[0]) || ('/' == pArg[0]))
		{
			...
			case OPT_FOLLOW_SRC_FILE_TYPE:
				needFollowSrcFile = true;
				break;
			...
			case OPT_COMPRESS_DITHER:
				...
				break;
			case OPT_RESIZE_PERCENTAGE:
				if (swscanf_s(pValue, L"%f", &resizePercentage) != 1)
				{
					wprintf(L"Invalid value specified with -aw (%ls)\n", pValue);
					wprintf(L"\n");
					PrintUsage();
				...
				else if (resizePercentage <= 0.0f)
				{
					wprintf(L"percentage must larger than 0, %ls", pValue);
					wprintf(L"\n");
					return 1;
				}
				break;
	...
	for (auto pConv = conversion.begin(); pConv != conversion.end(); ++pConv)
	{
		...
		size_t twidth = info.width;
		size_t theight = info.height;
		if (resizePercentage > 0)
		{
			if (info.width != 0)
				twidth = size_t(info.width * resizePercentage);

			if (info.height != 0)
				theight = size_t(info.height * resizePercentage);
		}
		else
		{
			if (0 != width)
		...
		// --- Save result -------------------------------------------------------------
		{
			...
			if (needFollowSrcFile)
			{
				...
				wchar_t* pchDotSrc = wcsrchr(pConv->szSrc, '.');				

				wcscat_s(pConv->szDest, pchDotSrc);
				wchar_t UpCh[16] = { 0 };
				wchar_t* pUpch = UpCh;
				for (wchar_t *p = pchDotSrc + 1; *p; ++p)
					*pUpch++ = wchar_t(::toupper(*p));

				CurFileType = LookupByName(UpCh, g_pSaveFileTypes);

				if (CurFileType != CODEC_DDS)
					nimg = 1;
			}
			else
			{
			...
			default:
				hr = SaveToWICFile(img, nimg, WIC_FLAGS_ALL_FRAMES, GetWICCodec(static_cast<WICCodecs>(CurFileType)), pConv->szDest);
				break;
			}
